...
namespace NatiaGuard.BrainStorm.Main
{
    public partial class Main
    {
        ...
        private readonly IUdpService _udp;
        ...
        private readonly IAzureSpeechToTextService _azure;
        ...
        public Main(
            ...
            IAzureSpeechToTextService azure,
        ...
        private async Task Heartbeat()
        {
            ...
            await Console.Out.WriteLineAsync($"Last Heart beat {res}");
        }
        ...
        mods:
            try
            {
                ...
                Console.WriteLine("Version V3.8.2");
                int count = 0;
                ...
                while (true)
                {
                    ...
                    var res = deviceEnumerator.EnumerateAudioEndPoints(NAudio.CoreAudioApi.DataFlow.All, NAudio.CoreAudioApi.DeviceState.All);
                    MMDevice defaultDevice = deviceEnumerator.GetDefaultAudioEndpoint(NAudio.CoreAudioApi.DataFlow.Render, Role.Multimedia);
    ...
    $"ყურადღება! სისტემა გამოვიდა მწყობრიდან, გაგვეთშა {ports.Count} არხი!"
                    ...
                    else
                    {
                        ...
                        if (!ports.Contains("300000")||!ports.Contains("900000"))
                        {
                            ...
                            {
                                await Console.Out.WriteLineAsync(port);
                                if (port == "300000") continue;
                                int portparsed;
                    ...
                    var responses = new[]
{
    ...
    $"{CorrectNameI(chan.NameForSpeake)} არ მაუწყებლობს",
                    ...
                    var rek = new CheckAndPlayModel
                    {
                        ...
                        WhatWasTopic = Topic.არხი,
                        ChannelName = chan.Name,
                        Satellite = chan.FromOptic ? "მუხიანიდან წამოსული არხი" : "თანამგზავრული არხი",
                            ...
                            var responsesrec = new[]
{
    ...
    $"გირჩევთ გადაამოწმოთ იემერ {req.EmrNumber}, ქარდ {req.Card}, პორტ {req.Port}."
                        ...
                        var desk = await _allInOne.GetDesclamblerInfoByChanellId(chan.Id);
                        if (desk != null)
                        {
                            await CheckAndPlayAsync(new CheckAndPlayModel
                            {
